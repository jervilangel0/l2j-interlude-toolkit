...
public class GeoEngine
{
	...
	/**
	 * Export a region's geodata to an L2D file. Writes all blocks using their
	 * native saveBlock() method, preserving flat/complex/multilayer fidelity.
	 * @param regionX : Tile X coordinate (e.g. 16-26)
	 * @param regionY : Tile Y coordinate (e.g. 10-25)
	 * @param outputPath : Full path for the output .l2d file
	 * @return true on success
	 */
	public boolean exportRegion(int regionX, int regionY, String outputPath)
	{
		if (regionX < World.TILE_X_MIN || regionX > World.TILE_X_MAX
			|| regionY < World.TILE_Y_MIN || regionY > World.TILE_Y_MAX)
		{
			LOGGER.warning("GeoEngine: exportRegion invalid tile " + regionX + "_" + regionY);
			return false;
		}

		final int baseBlockX = (regionX - World.TILE_X_MIN) * GeoStructure.REGION_BLOCKS_X;
		final int baseBlockY = (regionY - World.TILE_Y_MIN) * GeoStructure.REGION_BLOCKS_Y;
		...
		try (BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(outputPath)))
		{
			for (int ix = 0; ix < GeoStructure.REGION_BLOCKS_X; ix++)
			{
				for (int iy = 0; iy < GeoStructure.REGION_BLOCKS_Y; iy++)
				{
					final ABlock block = _blocks[baseBlockX + ix][baseBlockY + iy];
					if (block instanceof BlockNull)
					{
						// Null blocks have no geodata — write flat block at height 0
						bos.write(GeoStructure.TYPE_FLAT_L2D);
						bos.write(0);
						bos.write(0);
					}
					else
		...
		{
			LOGGER.warning("GeoEngine: Failed to export " + regionX + "_" + regionY + ": " + e.getMessage());
			return false;
		}
	}
